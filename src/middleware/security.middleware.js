...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    if (decision.isDenied()) {
      if (decision.reason.isBot()) {
        logger.warn("Shield Blocked request", { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path, methood:req.method });
      }

      return res.status(403).json({ error: 'Forbidden', message: 'Req blocked by sequrity policy' });
    }
    if (decision.isDenied()) {
